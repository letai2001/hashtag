...
def calculate_top_keywords_with_topic_2_es(es, input_date, data, index_name, platform):
    ...
    for topic_id in topic_ids:
        ...
        keyword_percentages = sorted(keyword_percentages, key=lambda x: x['percentage'], reverse=True)[:1000]

        hashtag_percentages = [
            {"hashtag": hashtag, "percentage": (count / total_articles) * 100, "record": count}
            for hashtag, count in hashtag_counts[topic_id].items()
        ]
        hashtag_percentages = sorted(hashtag_percentages, key=lambda x: x['percentage'], reverse=True)[:1000]
...
def calculate_top_keywords_with_trend_logic_topic(input_date, es, historical_data_index, platform):
    ...
    daily_keywords = [hit['_source'] for hit in daily_keywords]
    ##dataforcheckbig
    start_date_str = (input_datetime - timedelta(days=19)).strftime("%m_%d_%Y")
    
    results = es.search(index="keyword_a", body={
        "query": {
            "bool": {
                "filter": [
                    {"term": {"type": platform}},
    ...
    if len(daily_keywords) == 7:
        ...
        for topic_id, topic_data in current_day_keywords[0]['topic_ids'].items():
            ...
            un_top_keywords_2 = []
            black_keywords = []
            top_10_current_day_keywords = []

            if current_day_keywords:
                for topic_data in current_day_keywords[0]['topic_ids'].values():
                    top_10_current_day_keywords = [kw_info['keyword'] for kw_info in topic_data['keywords_top'][:4]]
            ...
            if len(top_keywords) > 400:
                top_keywords = top_keywords[:8]+top_keywords_big+top_keywords[8:400]   + un_top_keywords + un_top_keywords_2 + top_keywords[400:] + black_keywords
            else:
                try:
                    top_keywords = top_keywords[:8] + top_keywords_big + top_keywords[8:] + un_top_keywords + un_top_keywords_2 + black_keywords
                except:
                    top_keywords = top_keywords + top_keywords_big + un_top_keywords + un_top_keywords_2 + black_keywords
